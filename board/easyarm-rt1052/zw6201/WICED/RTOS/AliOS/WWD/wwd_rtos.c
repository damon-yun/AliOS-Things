...
//         return ( *thread == NULL ) ? WWD_THREAD_CREATE_FAILED : WWD_SUCCESS;
    ...
    else
    {
        ...
        result = aos_task_new_ext(thread, name, entry_function, (void *)arg, stack_size, priority);
        
        return ( result == 0 ) ? WWD_SUCCESS : WWD_THREAD_CREATE_FAILED;
    }
}
...
wwd_result_t host_rtos_init_semaphore(  /*@special@*/ /*@out@*/ host_semaphore_type_t* semaphore ) /*@allocates *semaphore@*/  /*@defines **semaphore@*/
{
    ...
    int ret = aos_sem_new(semaphore,0);
    /*@-compdef@*/ /* Lint doesnt realise allocation has occurred */
    return ( ret == 0 ) ? WWD_SUCCESS : WWD_SEMAPHORE_ERROR;
    /*@+compdef@*/
}
...
wwd_result_t host_rtos_get_semaphore( host_semaphore_type_t* semaphore, uint32_t timeout_ms, /*@unused@*/ wiced_bool_t will_set_in_isr ) /*@modifies internalState@*/
{
    ...
    ret = aos_sem_wait(semaphore, timeout_ms);
...
//    }
    ...
    int stat = aos_queue_new(queue, buffer, buffer_size ,message_size);
...
//    }
    retval = aos_queue_send(queue, message, sizeof(void *));
    if ( retval != 0 )
    {
        return WWD_QUEUE_ERROR;
    }
...
wwd_result_t host_rtos_pop_from_queue( host_queue_type_t* queue, void* message, uint32_t timeout_ms )
{
    int retval;
    unsigned int size;
    retval = aos_queue_recv(queue, timeout_ms, message, &size);
    if ( retval != 0 )
    {
        return WWD_QUEUE_ERROR;
    }
...
static StackType_t xIdleStack[WWD_THREAD_STACK_SIZE];
 
void vApplicationGetIdleTaskMemory(StaticTask_t **ppxIdleTaskTCBBuffer, StackType_t **ppxIdleTaskStackBuffer, uint32_t *pulIdleTaskStackSize) {
  *ppxIdleTaskTCBBuffer = &xIdleTaskTCBBuffer;
  *ppxIdleTaskStackBuffer = &xIdleStack[0];
  *pulIdleTaskStackSize = WWD_THREAD_STACK_SIZE;
}
#endif
