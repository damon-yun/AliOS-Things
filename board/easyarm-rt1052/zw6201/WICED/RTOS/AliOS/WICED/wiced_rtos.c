 ...
 ******************************************************/
void application_thread_main( void *arg );
...
//TaskHandle_t  app_thread_handle;
...
//#define xTaskGetCurrentThread  xTaskGetCurrentTaskHandle
...
{
    *time_ptr = (wiced_time_t) ( aos_now_ms() ) + wiced_time_offset;
    return WICED_SUCCESS;
}

wiced_result_t wiced_time_set_time( const wiced_time_t* time_ptr )
{
    wiced_time_offset = *time_ptr - (wiced_time_t) ( aos_now_ms() );
    return WICED_SUCCESS;
}
#if 0
...
//    }
    ...
    int ret = aos_timer_new(timer, function, arg, time_ms, 0);
    if ( ret != 0 )
    {
        return WICED_ERROR;
    }
...
//    if ( ((k_timer *)(timer->hdl))->timer_state == TIMER_ACTIVE ) {
//        return WICED_SUCCESS;
//    }
