...
wiced_result_t wiced_wlan_connectivity_init( void )
{
    ...
    WPRINT_NETWORK_DEBUG( ( "WWD SPI  interface initializing with %c%c/%d\n",
 ...
 #endif
{
    ...
    if ( !( NULL_MAC(details->BSSID.octet) ) && details->channel != 0 ) {
        join_result = (wiced_result_t) wwd_wifi_join_specific( &temp_scan_result, (uint8_t*) security_key, security_key_length, NULL, interface );
    }
    ...
    if ( join_result != WICED_SUCCESS ) {
        ...
        join_result = (wiced_result_t) wwd_wifi_join( &details->SSID, security, (uint8_t*) security_key, security_key_length, NULL, interface );
    }
    ...
    if ( join_result == WICED_SUCCESS ) {
        ...
        wwd_management_set_event_handler( link_events, wiced_link_events_handler, NULL, interface );
        return WICED_SUCCESS;
    } else {
...
//extern netifapi_netif_common(struct netif *netif, netifapi_void_fn voidfunc,
...
//        wiced_rtos_send_asynchronous_event( WICED_NETWORKING_WORKER_THREAD, wiced_wireless_link_up_handler, 0 );
        ...
        netifapi_netif_common( &IP_HANDLE(interface), invalidate_all_arp_entries, NULL );
